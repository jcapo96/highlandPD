//global declaration for comfortability
DrawingTools* draw = NULL;
...
//**********************************************
  ...
  for(int i = 0; i < sb_sample->GetSize(); i++){
    ...
    sb_sample->GetHistVector()[i]->GetYaxis()->SetTitleOffset(1.2);
  ...
  for(int i = 0; i < sb_sample->GetSize(); i++){
    ...
    sbd_sample->GetHistVector()[i]->GetYaxis()->SetTitleOffset(1.2);
...
//**********************************************
  ...
  for(int i = 0; i < sb_sample->GetSize(); i++){
    ...
    TH1F* h1 = (TH1F*)sb_sample->GetHistVector()[i]->Rebin(nbins,"Simulation",&edges[0]);
    h1->SetLineColor(98);
    ...
    //ratio style
    r->GetLowerRefYaxis()->SetTitle("(Sim-Data)/error_{Sim}");
  ...
  //canvas for drawing
  c = new TCanvas("c","c",700,500);
  
  //Print RR slices comparing data and MC using highland categories
  //histo style
  for(int i = 0; i < sb_sample->GetSize(); i++){
    //look for higher minimum
    double min1 = sb_sample->GetHistVector()[i]->GetBinCenter(1)-sb_sample->GetHistVector()[i]->GetBinWidth(1)/2;
    ...
    draw->SetTitleY("Probability density");
    draw->Draw(cf_d->GetTree(),cf_mc->GetTree(),
	       "bestcandidate_hit_dedx",nbins,min,max,
	       "bestcandidateparticlereduced",
	       ("accum_level[0]>2 && bestcandidate_hit_resrange>"+srmin.str()+" && bestcandidate_hit_resrange<"+srmax.str()+"").c_str(),
	       "","NORMBYWIDTH1  pur NOSTAT ignoreempty");
    ...
    std::stringstream ssh;
    ssh << i;
    c->Print(("plots/mc_d_slice_"+ssh.str()+"_categ.pdf").c_str());
  }
...
//**********************************************
  ...
  for(int i = 0; i < sb_sample->GetSize(); i++){
    ...
    double max = sb_sample->GetHistVector()[i]->GetBinCenter(nbins)+sb_sample->GetHistVector()[i]->GetBinWidth(nbins)/2;
    ...
    // draw->SetTitleX("dE/dx [MeV/cm]");
    // draw->SetTitleY("Probability density");
    // draw->Draw(cf_mc->GetTree(),
    // 	       "bestcandidate_hit_dedx",nbins,min,max,
    // 	       "bestcandidateparticlereduced",
    // 	       ("accum_level[0]>2 && bestcandidate_hit_resrange>"+srmin.str()+" && bestcandidate_hit_resrange<"+srmax.str()+"").c_str(),
    // 	       "","NORMBYWIDTH1 NOSTAT ignoreempty noleg");
    // f->Draw("same");
    // gPad->RedrawAxis();
    // tt1.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
    // tt2.DrawLatex(0.9,0.94,"Simulation");
    ...
    // // //Print 
    // gPad->Print(("plots/CF_mc_slice_"+ssh.str()+"_IF_categ.pdf").c_str());
    // gPad->Update();//gPad->WaitPrimitive();
...
//**********************************************
  ...
  f_ts_mpv->SetParameters(-3.96,0.15,5.61);
  tg_ts_mpv->Fit("f_ts_mpv","");
  tt1.DrawLatex(0.10,0.94,"#bf{DUNE:ProtoDUNE-SP}");
  gPad->Print("plots/CF_IF_mc_ts_mpv.pdf");
  gPad->Update();//gPad->WaitPrimitive();
...
//**********************************************
  ...
  mg->Draw("ap");
  gPad->BuildLegend(0.6,0.7,0.85,0.85,"","f");
  gPad->SetLogy();
  ...
  TLegend* lg = new TLegend(0.5,0.6,0.86,0.86);
  lg->AddEntry(tgmc,"Simulation Coherent Fit", "f");
  lg->AddEntry(tg_ts_lw,"Simulation Incoherent Fit", "lp");
  lg->Draw();
  ...
  diff->GetYaxis()->SetTitle("#sigma_{S,L} (Data-Sim)/#sqrt(#sigma_{Data}^{2}+#sigma_{Sim}^{2})");
  ...
  gPad->RedrawAxis();
  gPad->Update();
  gPad->Print("plots/CF_lw_comparative_differror.pdf");
...
//**********************************************
  ...
  TLegend* lg = new TLegend(0.5,0.6,0.86,0.86);
  lg->AddEntry(tgmc,"Simulation Coherent Fit", "f");
  lg->AddEntry(tg_ts_mpv,"Simulation Incoherent Fit", "lp");
  lg->Draw();
  ...
  diff->GetXaxis()->SetTitle("Residual Range [cm]");
  diff->GetYaxis()->SetTitle("(Data-Sim)/#sqrt(#sigma_{Data}^{2}+#sigma_{Sim}^{2})");
  diff->GetXaxis()->CenterTitle();
  diff->GetYaxis()->CenterTitle();
  diff->GetYaxis()->SetTitleOffset(0.9);
  diff->Draw("l");
  ...
  bb->SetLineWidth(3);
  bb->SetLineColor(1);
  bb->SetTitle("Bethe-Bloch prediction");
  bb->SetName("Bethe-Bloch prediction");

  tgmc->GetXaxis()->SetTitle("Residual Range [cm]");
  tgmc->GetYaxis()->SetTitle("MPV_{S} [MeV/cm]");
  tgmc->GetXaxis()->CenterTitle();
  tgmc->GetYaxis()->CenterTitle();
  tgmc->GetYaxis()->SetTitleOffset(0.9);
  tgmc->SetLineWidth(0);
  tgmc->Draw("a3");
  bb->Draw("samel");
  gPad->BuildLegend(0.5,0.7,0.85,0.85,"","lf");
  gPad->Print("plots/CF_mpv_comparative_mc_bethebloch.pdf");

  tgd->GetXaxis()->SetTitle("Residual Range [cm]");
  tgd->GetYaxis()->SetTitle("MPV_{S} [MeV/cm]");
  tgd->GetXaxis()->CenterTitle();
  tgd->GetYaxis()->CenterTitle();
  tgd->GetYaxis()->SetTitleOffset(0.9);
  tgd->SetLineWidth(0);
  tgd->Draw("a3");
  bb->Draw("samel");
  gPad->BuildLegend(0.5,0.7,0.85,0.85,"","lf");
  ...
  bb->SetLineWidth(3);
  bb->SetLineColor(1);
  bb->SetTitle("Landau-Vavilov prediction");
  bb->SetName("Landau-Vavilov prediction");

  tgmc->Draw("a3");
  bb->Draw("samel");
  gPad->BuildLegend(0.5,0.7,0.85,0.85,"","lf");
  gPad->Print("plots/CF_mpv_comparative_mc_landauvavilov.pdf");
...
//**********************************************
  ...
  TLegend* lg = new TLegend(0.5,0.6,0.86,0.86);
  lg->AddEntry(tgmc,"Simulation Coherent Fit", "f");
  lg->AddEntry(tg_ts_gw,"Simulation Incoherent Fit", "lp");
  lg->Draw();
  ...
  diff->GetXaxis()->SetTitle("Residual Range [cm]");
  diff->GetYaxis()->SetTitle("#sigma_{S,G} (Data-Sim)/#sqrt(#sigma_{Data}^{2}+#sigma_{Sim}^{2})");
  diff->GetXaxis()->CenterTitle();
  diff->GetYaxis()->CenterTitle();
  diff->GetYaxis()->SetTitleOffset(0.9);
  diff->Draw("al");
  ...
  gPad->RedrawAxis();
  gPad->Update();
  gPad->Print("plots/CF_gw_comparative_differror.pdf");
...
//**********************************************
  ...
  mg->Draw("ap");
  gPad->BuildLegend(0.6,0.7,0.85,0.85,"","l");
...
//**********************************************
void CoherentFitPlots_Extended(){        //Main function*
//**********************************************
  ...
  draw->ChangeCategoryColor("bestcandidateparticlereduced",321,217); //change kaon colors for candidates plots
