void DrawSyst_calE(){
  ...
  const int nbins = 11;
  double edges[nbins+1] = {0,100,200,300,400,500,600,800,1000,1500,2000,3000};

  //get MC nominal
  TH1F* h_mc_nominal = new TH1F("h_mc_nominal","h_mc_nominal",nbins,edges);
  syst->Project("h_mc_nominal","bestcandidate_calE_toy","(accum_level>2 && bestcandidate_chi2_kaon_perndf_toy<50)*weight_syst_total*toy_weight");
  ...
  draw.DrawRelativeErrors(syst,"bestcandidate_calE_toy",nbins,edges,"accum_level>2 && bestcandidate_chi2_kaon_perndf_toy<50","","SYS");
  ...
  d->Project("h_data","bestcandidate_calE","accum_level>2 && bestcandidate_chi2_kaon/bestcandidate_chi2_ndf<50");
  ...
  h_mc_syst->GetXaxis()->SetTitle("K [MeV]");
  ...
  gPad->Print("SYST_final_kaon_calE.pdf");
}
