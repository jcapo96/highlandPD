...
std::vector< float >  protoana::ProtoDUNECalibration::GetCalibratedCalorimetry(  const recob::Track &track, const std::string trackModule, const std::string caloModule ) {


  std::vector< float > calibrated_dEdx;

  //Get the Calorimetry vector from the track
  std::vector< anab::Calorimetry > caloVector = trackUtil.GetRecoTrackCalorimetry( track, trackModule, caloModule ); 
...
double protoana::ProtoDUNECalibration::HitToEnergy(recob::Hit* hit, double X, double Y, double Z){

  //Only do collection plane
  if( hit->fView != 2 ) return 0.;
