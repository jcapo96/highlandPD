...
//********************************************************************
dEdxVariation::dEdxVariation():EventVariationBase(),BinnedParams(std::string(getenv("PDSYSTEMATICSROOT"))+"/data","dEdx", BinnedParams::k1D_SYMMETRIC_NOMEAN){
//********************************************************************
...
//********************************************************************
  ...
  for(Int_t ipart = 0; ipart < box->nRelevantRecObjects; ipart++){
    ...
    for(int ihit = 0; ihit < (int)part->Hits[2].size(); ihit++){
      if(!GetBinValues(0.5, params, index))return;
      double width = params.sigma;
      //get the calibration
      part->Hits[2][ihit].dEdx = part->Hits[2][ihit].dEdx * (1 + width*toy.GetToyVariations(_index)->Variations[index]);
      //recompute dEdx
    }
...
//**************************************************
bool dEdxVariation::IsRelevantRecObject(const AnaEventC& event, const AnaRecObjectC& part) const{
//**************************************************
