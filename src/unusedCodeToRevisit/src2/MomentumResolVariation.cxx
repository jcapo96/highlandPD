...
//********************************************************************
  ...
  Float_t pt0_inv_true  = anaUtils::ComputeInversePT(*(track->GetTrueParticle())); 
    ...
    if (track->nSubdet2Segments == 0) return;
    ...
    for(int iseg = 0; iseg < track->nSubdet2Segments; iseg++){
      AnaSubdet2ParticleB* tpcTrackOrig = static_cast<const AnaTrackB*>(track->Original)->Subdet2Segments[iseg];
      AnaSubdet2ParticleB* tpcTrack     = track->Subdet2Segments[iseg];
...
//********************************************************************
  ...
  AnaSubdet2ParticleB* Subdet2Segments[3];
  Int_t nSubdet2Segments = anaUtils::GetOneSegmentPerSubdet2(track->Subdet2Segments, track->nSubdet2Segments, Subdet2Segments);

  // Loop over TPC segments
  for (int k = 0; k < nSubdet2Segments; k++) {
    AnaSubdet2ParticleB* tpcTrack = Subdet2Segments[k];
