...
void TPCPIDVariation::ApplyVariation(AnaTrackB* track, const ToyExperiment& exp){
  ...
  for (int k = 0; k < track->nSubdet2Segments; k++) {

    if (!track->Subdet2Segments[k]) continue;

    // The segment to be modified
    AnaSubdet2ParticleB* tpcTrack = track->Subdet2Segments[k];
    ...
    const AnaSubdet2ParticleB* original = static_cast<const AnaSubdet2ParticleB*>(tpcTrack->Original);
...
//**************************************************
bool TPCPIDVariation::GetPIDParams(const AnaSubdet2ParticleB& tpcTrack, 
