...
bool GeometryManager::LoadGeometry(const std::string& file_name0, Int_t geomID, const std::string& geomDir0) {
  ...
  */
  ...
  if (ND::params().GetParameterI("psycheDUNEUtils.Geometry.DetectorDefinitionFromROOTGeometry"))
...
//*****************************************************************************
void GeometryManager::FillSubdet1InfoFromROOTGeometry() const {
  //*****************************************************************************
...
bool GeometryManager::VisitNode(const std::string& name, const TGeoNode* node) const {
  ...
  if (name.find("Subdet2_1")     != std::string::npos
    //  && name.find("Drift") != std::string::npos) {
    ){
    UpdateVolumeBound(DetDef::Subdet2_1min, DetDef::Subdet2_1max, low, high);
    return false;
  }
  if (name.find("Subdet2_2")     != std::string::npos
     // && name.find("Drift") != std::string::npos) {
    ){
    UpdateVolumeBound(DetDef::Subdet2_2min, DetDef::Subdet2_2max, low, high);
    return false;
  }
  if (name.find("Subdet2_3")     != std::string::npos
   //   && name.find("Drift") != std::string::npos) {
  ){
    UpdateVolumeBound(DetDef::Subdet2_3min, DetDef::Subdet2_3max, low, high);
    return false;
  }

  //SUBDET1_1 
  if (name.find("SUBDET1_1")       != std::string::npos
      && name.find("/Scint")  != std::string::npos) {
    UpdateVolumeBound(DetDef::Subdet1_1min, DetDef::Subdet1_1max, low, high);
    return false;
  }

  //SUBDET1_2
  if (name.find("SUBDET1_2")       != std::string::npos
      && name.find("/Scint")  != std::string::npos) {
    UpdateVolumeBound(DetDef::Subdet1_2min, DetDef::Subdet1_2max, low, high);
    return false;
  }
...
void GeometryManager::ResetVolumeDefinitions() const {
  ...
  //SUBDET1_1
  SetDetectorBoundaries(DetDef::Subdet1_1min,      DetDef::Subdet1_1max,    min, max); 

  //SUBDET1_2
  SetDetectorBoundaries(DetDef::Subdet1_2min,      DetDef::Subdet1_2max,    min, max); 

  //Subdet2_1
  SetDetectorBoundaries(DetDef::Subdet2_1min,      DetDef::Subdet2_1max,    min, max); 

  //Subdet2_2
  SetDetectorBoundaries(DetDef::Subdet2_2min,      DetDef::Subdet2_2max,    min, max); 

  //Subdet2_3
  SetDetectorBoundaries(DetDef::Subdet2_3min,      DetDef::Subdet2_3max,    min, max);
